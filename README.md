# Aryan Prasad

### 📊 Aspiring Data Analyst | Power BI • SQL • Python • Excel

Transforming raw data into actionable business insights through analytics, dashboards, and data-driven decision making.

<p align="center">

<a href="https://www.linkedin.com/in/prasadaryan9354">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/thearyanprasad">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://thearyanprasad.github.io">
<img src="https://img.shields.io/badge/Portfolio-00C853?style=for-the-badge&logo=google-chrome&logoColor=white"/>
</a>

<a href="https://thearyanprasad.github.io/assets/Aryan_Prasad_Resume.pdf">
<img src="https://img.shields.io/badge/Resume-6A1B9A?style=for-the-badge&logo=readthedocs&logoColor=white"/>
</a>

</p>

---

# 🚀 About Me

- 🎓 BCA Student at IGNOU (Expected Graduation: 2027)
- 📊 Passionate about Data Analytics, Business Intelligence & Visualization
- 💼 Completed a 6-Month Data Analytics Internship at Eagletfly Solutions Pvt Ltd
- 📈 Skilled in Power BI, SQL, Excel, Python, and Dashboard Development
- 🚀 Building real-world analytics projects focused on business insights
- 🎯 Seeking opportunities in Data Analytics and Business Intelligence

---

# 🛠 Tech Stack

### Analytics & Visualization
`Power BI` • `Excel` • `Dashboard Design`

### Database & Querying
`MySQL` • `SQL`

### Programming
`Python`

### Tools
`GitHub` • `VS Code`

<p align="center">
<img src="https://skillicons.dev/icons?i=python,mysql,github,vscode" />
</p>

---

# 📂 Featured Projects

## 📊 LV Analytics Dashboard

<p align="center">
<img src="https://raw.githubusercontent.com/thearyanprasad/LV-Analytics-Dashboard/main/dashboard1.png" width="100%">
</p>

### Project Overview

Interactive Power BI dashboard developed to monitor KPIs, analyze business performance, and generate actionable insights.

### Key Features

- KPI Monitoring
- Sales Performance Analysis
- Trend Analysis
- Interactive Filters
- Executive Reporting Dashboard

### Tools

`Power BI` • `SQL` • `Excel`

🔗 Repository:  
https://github.com/thearyanprasad/LV-Analytics-Dashboard

---

## ⚡ Pokémon Excel Dashboard

<p align="center">
<img src="https://raw.githubusercontent.com/thearyanprasad/pokemon-excel-dashboard/main/dashboard_preview_excel.png" width="100%">
</p>

### Project Overview

Excel-based interactive dashboard built using Pivot Tables, KPI Cards, Dynamic Slicers, and Charts.

### Key Features

- Pokémon Type Analysis
- Generation-wise Insights
- Legendary vs Normal Comparison
- Interactive Filters
- KPI Reporting

### Tools

`Excel` • `Pivot Tables` • `Dashboard Design`

🔗 Repository:  
https://github.com/thearyanprasad/pokemon-excel-dashboard

---

## 🗃 SQL Sales Analysis Project

### Project Overview

Business-oriented SQL project performed on an E-Commerce Sales dataset to extract valuable insights using SQL queries.

### Analysis Performed

- Revenue Analysis
- Top Product Analysis
- Regional Performance
- Customer Insights
- Monthly Sales Trends
- Profitability Analysis

### Tools

`MySQL` • `SQL`

🔗 Repository:  
https://github.com/thearyanprasad/sql-sales-analysis

---

# 💼 Experience

## Data Analytics Industrial Internship

### Eagletfly Solutions Pvt Ltd

**Duration:** 6 Months

#### Responsibilities

- SQL Data Extraction & Analysis
- Dashboard Development using Power BI
- Data Cleaning & Transformation
- Business Reporting
- Data Visualization

#### Technologies

`Power BI` • `SQL` • `Python` • `Excel`

---

# 📚 Education

## Bachelor of Computer Applications (BCA)

**Indira Gandhi National Open University (IGNOU)**

Expected Graduation: **2027**

---

# 🏆 Certifications & Achievements

- Data Analytics Industrial Internship – Eagletfly Solutions Pvt Ltd
- Developed Interactive Power BI Dashboards
- Built Pokémon Analytics Dashboard in Excel
- Completed SQL Sales Analysis Project
- Published Personal Portfolio Website

---

# 🌱 Currently Learning

- Advanced SQL
- Power BI DAX
- Python for Analytics
- Business Intelligence
- Data Storytelling
- Statistical Analysis

---

# 📈 GitHub Stats

<p align="center">

<img src="https://github-readme-stats.vercel.app/api?username=thearyanprasad&show_icons=true&theme=tokyonight"/>

<br><br>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=thearyanprasad&theme=tokyonight"/>

<br><br>

<img src="https://github-profile-trophy.vercel.app/?username=thearyanprasad&theme=tokyonight&margin-w=10"/>

</p>

---

# 🌐 Connect With Me

<p align="center">

<a href="mailto:prasadaryan.work@gmail.com">📧 Email</a> •
<a href="https://github.com/thearyanprasad">💻 GitHub</a> •
<a href="https://www.linkedin.com/in/prasadaryan9354">🔗 LinkedIn</a> •
<a href="https://www.instagram.com/madmello_aryan">📸 Instagram</a>

</p>

---

# ⚡ Fun Fact

> I enjoy turning raw datasets into interactive dashboards that help businesses make smarter decisions.

⭐ If you like my work, feel free to connect with me and explore my projects.
